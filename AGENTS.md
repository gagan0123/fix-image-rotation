...
scripts/
  readme-to-markdown.js                # Converts readme.txt to README.md with icon and screenshot URLs
package.json                           # NPM config and build scripts (readme, makepot)
...
npm scripts handle build tasks:

- **`lando npm run readme`** — Converts `readme.txt` to `README.md` (via `scripts/readme-to-markdown.js`)
- **`lando npm run makepot`** — Generates POT translation file (via WP-CLI `wp i18n make-pot`)

The readme script prepends the plugin icon and rewrites screenshot URLs, matching the previous Grunt task behavior.
...
- `README.md` — `Stable tag:` field (auto-generated by `npm run readme`)
...
Lando is used for local development (`lando start`). All commands run inside the container — do NOT use bare `npm`, `node`, or `php` on the host.
...
- After editing `readme.txt`, run `lando npm run readme` and commit the updated `README.md`
